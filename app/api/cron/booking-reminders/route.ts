    ...
    if (options.includeBorrower && row.borrower_id) {
      ...
      if (reserved) {
        await sendReminder({
          ...
          cronSecret,
          userId: row.borrower_id,
          url: options.borrowerUrl || "/dashboard/borrower",
          requestId: row.id,
          eventType: options.eventType,
    ...
    if (options.includeOwner && ownerId) {
      ...
      if (reserved) {
        await sendReminder({
          ...
          cronSecret,
          userId: ownerId,
          url: options.ownerUrl || "/dashboard/owner/requests",
          requestId: row.id,
          eventType: options.eventType,
          recipientRole: "owner",
        });
...
export async function GET(req: Request) {
  try {
    ...
    const [startsTomorrowRows, startsTodayRows, endsTomorrowRows, pendingRows] =
      await Promise.all([
    ...
    const pendingOwnerReminders = await processCandidates(
      admin,
      baseUrl,
      cronSecret,
      pendingRows,
    ...
    return Response.json({
      ...
      results: {
        ...
        pending_request_owner_reminder: pendingOwnerReminders,
      },
    });
  } catch (e: any) {
